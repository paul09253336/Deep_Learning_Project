...
def GenerateRandomCurves(X, sigma=0.2, knot=4):
    ...
    yy = np.random.normal(loc=1.0, scale=0.2, size=(knot+2, X.shape[1]))
    x_range = np.arange(X.shape[0])
    cs_x = CubicSpline(xx[:,0], yy[:,0])
    #print(yy)
    # plt.figure(0)
    # plt.plot(np.array([cs_x(x_range)]).transpose())
    # plt.show()
    # sys.exit()
...
if __name__ == '__main__':
    ...
    print(traindataA1.shape,"\ntest",traindataN1.shape)
    #sys.exit()
    traindataN2 = []
    ...
    ###############################
    label1= np.zeros(trainA)
    label_b= np.zeros(trainA)#label2 = np.ones(trainN-2123+1)
    ...
    print(y)
    # ######for class_w
    X = np.concatenate((traindataN2,traindataA2), axis=0)


    ######for balance
    # X_trainA, X_vaildA, y_trainA, y_vaildA =train_test_split(traindataA2, label1, test_size=0.2,random_state=52)
    # X_trainN, X_vaildN, y_trainN, y_vaildN = train_test_split(traindataN2, label3, test_size=0.2, random_state=52)
    #
    # print(X_trainN.shape,X_trainA.shape)
    # diff=X_trainN.shape[0]-X_trainA.shape[0]
    # print(diff)
    # random_number = random.sample(range(0, len(X_trainA)), int(diff))
    # randomA = X_trainA[random_number]
    # random_label=np.zeros(diff)
    # print(randomA.shape)
    # #sys.exit()
    #
    # X_train = np.concatenate((X_trainN, X_trainA,randomA), axis=0)
    # X_vaild=np.concatenate((X_vaildN, X_vaildA), axis=0)
    # y_train=np.concatenate((y_trainN, y_trainA,random_label), axis=0)
    # y_vaild=np.concatenate((y_vaildN, y_vaildA), axis=0)
    ...
    print(X_train_noise[10][0].shape)
    #sys.exit()
    #plt.figure(1)
    lim = np.linspace(0, 1500 - 1, num=1500, endpoint=True)
    # plt.subplot(4, 1, 1)
    # plt.plot(lim,X_train[11][0][0:1000])
    #
    # plt.subplot(4, 1, 2)
    # plt.plot(lim, X_train_noise[11][0][0:1000])
    # plt.subplot(4, 1, 3)
    # plt.plot(lim, X_train_noise2[11][0][0:1000])
    # plt.subplot(4, 1, 4)
    # plt.plot(lim, X_train_noise3[11][0][0:1000])
    #
    # plt.show()
    #print(X_train_noise[10].shape)
    ...
    #
    # plt.figure(1)
    # plt.plot(lim, X_train[11][0][0:1500], lim, X_train_wrap5[11][0][0:1500])
    # plt.legend(['Orignal', 'Time Warp'], loc='upper right')
    # plt.show()
    # sys.exit()
    ...
    #X_train=np.concatenate((X_train,X_train_wrap,X_train_noise_warping,X_train_noise_warping2,X_train_wrap3,X_train_wrap4),axis=0)
   ...
   #####Train
    ...
    for epoch in range(4):
        ...
        apnea_ecgname = dataset_name + '_cnn_v21_n0.2'
        save_models_path = os.getcwd() + "/models_save/StandardScaler_cnn_combinfv2/"
        ...
        # train_history = train_model.fit(X_train, train_y_onehot, batch_size=(256), epochs=200,validation_data=(X_vaild,valid_y_onehot),shuffle=True
        #                                 ,class_weight=class_w,callbacks=callbacks_list)
        train_history = train_model.fit(X_train, train_y_onehot, batch_size=(512), epochs=300,
                                        validation_data=(X_vaild, valid_y_onehot), shuffle=True,class_weight=class_w)#, callbacks=callbacks_list)
        ...
        save_file_name=os.getcwd()+'/fig_and_other/cnn+lstm_balance/'+ apnea_ecgname+'.'+str(epoch)
